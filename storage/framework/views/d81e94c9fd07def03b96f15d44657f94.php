...
<body>
        ...
        <nav class="navbar">
            <div class="branding">
                <div class="brand-icon">
                    <img src="<?php echo e(asset('images/LOGO_SMKN_4.png')); ?>" alt="SMKN 4 Bogor Logo" style="width: 100%; height: 100%; object-fit: contain;">
                </div>
            ...
            <div class="nav-menu">
                <ul class="nav-links">
                    ...
                    <li><a href="<?php echo e(route('user.gallery')); ?>" class="active">Galeri</a></li>
        ...
        <div class="container">
            ...
            <div class="gallery-controls">
                <div class="search-filter">
                    ...
                    <select class="filter-select" id="categoryFilter">
                        ...
                        <option value="<?php echo e($kategori->judul); ?>"><?php echo e($kategori->judul); ?></option>
            ...
            <div class="gallery-grid" id="galleryGrid">
                ...
                <div class="gallery-item" 
                     data-category="<?php echo e($item->post->kategori->judul ?? 'Umum'); ?>" 
                     data-gallery-id="<?php echo e($item->id); ?>"
                     <?php if($item->fotos->count() > 0): ?>
                        data-foto-id="<?php echo e($item->fotos->first()->id); ?>"
                        data-foto-file="<?php echo e($item->fotos->first()->file); ?>"
                        data-foto-likes="<?php echo e($item->fotos->first()->likes); ?>"
                        data-foto-dislikes="<?php echo e($item->fotos->first()->dislikes ?? 0); ?>"
                     <?php endif; ?>>
                    <?php if($item->fotos->count() > 0): ?>
                        <img src="<?php echo e(asset('uploads/galeri/' . $item->fotos->first()->file)); ?>" 
                             alt="<?php echo e($item->post->judul ?? 'Gallery Image'); ?>" 
                             class="gallery-image"
                             onclick="openGalleryModal('<?php echo e($item->id); ?>')">
                    <?php else: ?>
                    ...
                    <div class="gallery-info">
                        <h3 class="gallery-title"><?php echo e($item->post->judul ?? 'Untitled'); ?></h3>
                        <div class="gallery-meta-row">
                            <span class="gallery-category">
                                <span class="category-tag-icon">
                                    <i class="fas fa-tag"></i>
                                </span>
                                <span class="category-text"><?php echo e($item->post->kategori->judul ?? 'Umum'); ?></span>
                            </span>
                            <span class="gallery-meta-separator">•</span>
                            <span class="gallery-photo-count">
                                <i class="fas fa-image"></i> <?php echo e($item->fotos->count()); ?> foto
                            </span>
                        </div>
                        <p class="gallery-description"><?php echo e($item->post->isi ?? 'No description available'); ?></p>
                        <?php if($item->fotos->count() > 0): ?>
                            <?php $defaultTitle = "Gallery Image"; ?>
                            <div class="gallery-actions">
                                <button class="gallery-action-btn btn-like-action" data-type="like" data-foto-id="<?php echo e($item->fotos->first()->id); ?>" onclick="event.stopPropagation(); toggleLike('<?php echo e($item->fotos->first()->id); ?>', this)">
                                    <i class="fas fa-thumbs-up action-icon"></i>
                                    <span class="action-count"><?php echo e($item->fotos->first()->likes); ?></span>
                                </button>

                                <button class="gallery-action-btn btn-dislike-action" data-type="dislike" data-foto-id="<?php echo e($item->fotos->first()->id); ?>" onclick="event.stopPropagation(); toggleDislike('<?php echo e($item->fotos->first()->id); ?>', this)">
                                    <i class="fas fa-thumbs-down action-icon"></i>
                                    <span class="action-count"><?php echo e($item->fotos->first()->dislikes ?? 0); ?></span>
                                </button>

                                <button class="gallery-action-btn btn-download-action" onclick="event.stopPropagation(); openDownloadModal('<?php echo e($item->fotos->first()->file); ?>', '<?php echo e(addslashes($item->post->judul ?? $defaultTitle)); ?>')">
                    ...
                    <div class="gallery-photos-data" style="display: none;">
                        <?php $__currentLoopData = $item->fotos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $foto): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <span data-photo="<?php echo e(asset('uploads/galeri/' . $foto->file)); ?>"></span>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </div>
                </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    ...
    <script>
        ...
        async function openDownloadModal(filePath, fileName) {
            ...
            // Generate captcha
            try {
                const response = await fetch("<?php echo e(route('download.captcha')); ?>", {
                    method: 'POST',
                    headers: {
        ...
        async function verifyAndDownload() {
            ...
            try {
                const response = await fetch("<?php echo e(route('download.verify')); ?>", {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
...
<?php /**PATH /Users/tanziljws/Downloads/ujikom/resources/views/user/gallery.blade.php ENDPATH**/ ?>