...
<div class="w-full">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('admin.informasi-items.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md text-sm font-medium">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse ($informasiItems as $item)
                    <tr>
                        ...
                        <td class="px-4 py-3 text-sm text-gray-500">{{ optional($item->date)->format('d M Y') }}</td>
                        ...
                        <td class="px-4 py-3 text-sm text-gray-500">{{ $item->order }}</td>
                        <td class="px-4 py-3 text-sm text-right space-x-2">
                            <a href="{{ route('admin.informasi-items.edit', $item->id) }}" class="inline-flex items-center px-3 py-1 border border-transparent text-xs leading-4 font-medium rounded-md text-white bg-yellow-500 hover:bg-yellow-600">
                                <i class="fas fa-edit mr-1"></i>Edit
                            </a>
                            <form action="{{ route('admin.informasi-items.destroy', $item->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin ingin menghapus informasi ini?');">
                                @csrf
                                @method('DELETE')
